...
class ModuloTipo3:
    ...
    def _crear_interfaz(self):
        titulo = ctk.CTkLabel(self.frame_principal, text="Gestión de Tipo 3", font=("Arial", 16, "bold"))
        titulo.pack(pady=10)
    ...
    def _cargar_datos(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT id, nombre, descripcion FROM tipo3")
    ...
    def _guardar(self):
        ...
        try:
            ...
            conn = db.connect_mysql()
            cursor = conn.cursor()
            cursor.execute("INSERT INTO tipo3 (nombre, descripcion) VALUES (%s, %s)", (nombre, descripcion))
            conn.commit()
            conn.close()
    ...
    def _eliminar_registro(self):
        ...
        id_tipo3 = self.tree.item(item[0], "values")[0]
        ...
        if confirmacion:
            try:
                ...
                conn = db.connect_mysql()
                cursor = conn.cursor()
                cursor.execute("DELETE FROM tipo3 WHERE id = %s", (id_tipo3,))
                conn.commit()
                conn.close()
                self._cargar_datos()
